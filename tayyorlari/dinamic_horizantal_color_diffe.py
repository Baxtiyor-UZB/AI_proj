...
def update_bar(i):
    ...
    colors = color_map(np.linspace(0, 1, len(sorted_values)))  # Generate colors for the bars
    bars = ax.barh(y, sorted_values, color=colors)  # Create horizontal bars with sorted positions and values
    ax.set_xlim(0, 15)
    ax.set_yticks(y)
